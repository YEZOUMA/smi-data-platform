...
EXTRACTION_COUNTER = Counter(
    "smi_extraction_total", "Nombre total d extractions effectuées", ["status"]
)
EXTRACTION_DURATION = Histogram("smi_extraction_duration_seconds", "Durée d extraction en secondes")
RECORDS_EXTRACTED = Counter(
    "smi_records_extracted_total", "Nombre total d enregistrements extraits"
)
...
class ExcelExtractor:
    ...
    def extract(self) -> pd.DataFrame:
        ...
        try:
            ...
            # Logs d'information
            logger.info(
                f"✅ Extraction réussie: {len(self.df)} lignes, " f"{len(self.df.columns)} colonnes"
            )
